...
class ContainerTest extends AnyWordSpec with Matchers {
  ...
  "Container.systemAndContainer" should {
    ...
    /** this is handy just to see what Enclosing does for various approaches:
      *
      * as it turns out, the text can be split on whitespace, a hash, or a dot, depending on being
      * in an object, inner function, etc:
      *
      * kind.logic.ContainerTest.nested.innerObj.example kind.logic.ContainerTest.nested.API#foo
      * kind.logic.ContainerTest#inTest kind.logic.ContainerTest#nestedFunc foo
      */
    ...
    "use the last two elements from a path" in {
      ...
      Container.systemAndContainer(using
        sourcecode.Enclosing("kind.logic.ContainerTest.nested.API#foo")
      ) shouldBe ("API", "foo")
      Container.systemAndContainer(using
        sourcecode.Enclosing("kind.logic.ContainerTest#nestedFunc foo")
      ) shouldBe ("nestedFunc", "foo")
    }
  }
}
