...
class App extends AnyWordSpec with Matchers {
  ...
  object assets {
    ...
    object Onboarding {
      val System = Actor.svc
      val Database = Actor.database("assets", "db")

      def apply()(using t : Telemetry) : Onboarding = new Onboarding {
        def create(spreadsheet: Spreadsheet): Task[ApplicationId] = "id".asTaskTraced(System, Database, spreadsheet)

        def list(): Task[Map[ApplicationId, Spreadsheet]] = Map("1" -> testSpreadsheet).asTaskTraced(System, Database)
  ...
  object marketplace {
    ...
    case class ExecutionReport(buyer : uim.UserId, seller : uim.UserId, asset : assets.AssetId, quantity : Int)
    trait Exchange {
