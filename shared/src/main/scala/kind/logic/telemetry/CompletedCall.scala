...
final case class CompletedCall(
    callId: Long,
    responseId: Option[Long],
    invocation: CallSite,
  ...
  def toStringMonocolor =
    s"${timestamp.asNanos}@$callId: $source --[ $operation ]--> $target $resultText and took ${duration
        .map(_.toString)
        .getOrElse("N/A")} at $atDateTime"
