...
case class Mermaid(calls: UIO[Seq[CompletedCall]]) {
  ...
  def asCallStack(all: Seq[CompletedCall]): Seq[SendMessage] = {
    val sortedCalls     = all.sortBy(_.timestamp.asNanos)
    val sortedCompleted = sortedCalls.flatMap(_.inverse).sortBy(_.timestamp.asNanos)
    SendMessage.fromCalls(sortedCalls, sortedCompleted)
  }

  def callStack: UIO[Seq[SendMessage]] = calls.map(asCallStack)

  // .map(_.asMermaidString(maxLenComment, maxComment))
